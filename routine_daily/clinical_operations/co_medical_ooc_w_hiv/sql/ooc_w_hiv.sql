...
IF OBJECT_ID('tempdb..#willow_information') IS NOT NULL DROP TABLE #willow_information;
WITH
    willow_info AS (
        SELECT ORDER_MED.PAT_ID,
               CAST(ORDER_DISP_INFO.FILL_SERVICE_DATE AS DATE) AS FILL_SERVICE_DATE,
               ROW_NUMBER() OVER (PARTITION BY ORDER_MED.ORDER_MED_ID
                                  ORDER BY ORDER_DISP_INFO.FILL_SERVICE_DATE DESC) AS ROW_NUM_DESC
        FROM Clarity.dbo.ORDER_DISP_INFO_VIEW AS ORDER_DISP_INFO
            INNER JOIN Clarity.dbo.ORDER_MED_VIEW AS ORDER_MED ON ORDER_DISP_INFO.ORDER_MED_ID = ORDER_MED.ORDER_MED_ID
        ...
        WHERE ORDER_DISP_INFO.FILL_TYPE_C NOT IN ( 4, 3 )
              AND ORDER_DISP_INFO.ORD_CNTCT_TYPE_C = 11
              AND ORDER_DISP_INFO.FILL_STATUS_C IN ( 80, 81, 82 ) -- Dispensed, Shipped, Delivered
              AND DATEDIFF(MONTH, ORDER_DISP_INFO.FILL_SERVICE_DATE, GETDATE()) <= 12
    )
SELECT willow_info.PAT_ID,
       MAX(willow_info.FILL_SERVICE_DATE) AS MOST_RECENT_RX_FILL
INTO #willow_information
FROM willow_info
WHERE willow_info.ROW_NUM_DESC = 1
GROUP BY willow_info.PAT_ID
ORDER BY willow_info.PAT_ID;
...
WHERE pev.CONTACT_DATE > DATEADD(MONTH, -80, GETDATE()) --can't do this since looking for pts not seen in a long time
...
WHERE a2.ROW_NUM_DESC = 1
      AND a2.PAT_ID IN ( SELECT DISTINCT pev.PAT_ID
                         ...
                         WHERE ser.SERV_AREA_ID = 64
                               AND ser.PROVIDER_TYPE_C IN ( '1', '9', '6', '113' ) -- Physicians and NPs
...
SELECT ftt.MRN,
       ...
       ftt.PROV_NAME PCP,
       ftt.PAT_NAME,
       CAST(ftt.LAST_OFFICE_VISIT AS DATE) AS LAST_OFFICE_VISIT,
       DATEDIFF(MONTH, ftt.LAST_OFFICE_VISIT, GETDATE()) 'MONTHS AGO',
       CAST(ftt.BIRTH_DATE AS DATE) AS BIRTH_DATE,
       zpr.NAME RACE,
       zs.NAME SEX,
       mr.CITY,
       mr.STATE,
       CAST(mr.MOST_RECENT_RESULT_DATE AS DATE) AS MOST_RECENT_RESULT_DATE,
       mr.MOST_RECENT_Result_Output,
       mr.ORD_VALUE
INTO #a
FROM #First_Temp_Table ftt
    LEFT JOIN #MOST_RECENT mr ON mr.MRN = ftt.MRN
...
CASE WHEN a.MOST_RECENT_Result_Output IS NULL THEN 'NO VL DATA'
    WHEN a.MOST_RECENT_Result_Output < 200 THEN 'SUPPRESSED'
    ELSE 'UNSUPPRESSED'
END AS VL_STATUS,
a.ORD_VALUE,
...
FROM #b b
    LEFT JOIN (SELECT pev.PAT_ID,
               ...
               WHERE pev.APPT_STATUS_C = 1
                     AND ser.PROVIDER_TYPE_C IN ( '1', '9', '113', '6' )
                     AND ser.PROV_ID <> '640178' --pulmonologist
    ) na ON b.PAT_ID = na.PAT_ID
            AND na.ROW_NUM_ASC = 1
    LEFT JOIN #willow_information ON b.PAT_ID = #willow_information.PAT_ID
