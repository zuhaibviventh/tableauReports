...
SELECT c.PAT_ID,
       ...
       MIN(CASE WHEN icd10.CODE LIKE 'F31.%' --Bipolar, Schizophrenia
                     ...
                     OR icd10.CODE = 'F06.2'
                     OR icd10.CODE = 'F28'
                     OR icd10.CODE = 'F29' THEN 'Bipolar, Schizophrenia, Psychosis'
               ELSE 'Depression'
...
SELECT 
	...
	,'Bipolar, Schizophrenia, Psychosis' 'Psychiatric Diagnosis Type'
...
FROM 
	...
	INNER JOIN Clarity.dbo.GROUPER_COMPILED_REC_LIST AS gcrl ON gcrl.GROUPER_RECORDS_NUMERIC_ID = edg.DX_ID

WHERE
	gcrl.BASE_GROUPER_ID IN ('21082919') --HP EDG CONCEPT: PSYCHOSIS
...
WHERE (icd.CODE LIKE 'F11%'--
      -- OR icd.CODE LIKE 'F12%' - Marajuana
...
;
SELECT c.PAT_ID,
       MAX(CASE WHEN sud.[Substance Use Disorder] IS NOT NULL THEN 3
               WHEN s.[Substance Use Disorder] IS NOT NULL THEN 1
               ELSE 0
           END) AS 'Substance Use',
       MIN(CASE WHEN sud.[Substance Use Disorder] IS NOT NULL THEN 'Cocaine, Opioid, Meth'
               WHEN s.[Substance Use Disorder] IS NOT NULL THEN 'Alcohol'
               ELSE NULL
           END) AS 'Substance'
INTO #SUD_SUM
FROM #COHORT c
    LEFT JOIN #alc s ON s.PAT_ID = c.PAT_ID
...
SELECT mc.PAT_ID,
       mc.MED_COUNT,
       CASE WHEN mc.MED_COUNT < 4 THEN 0
			WHEN mc.MED_COUNT < 10 THEN 1
           WHEN mc.MED_COUNT >= 10 THEN 3
       END AS NUM_OF_MEDS
INTO #MEDGRP
FROM #MEDS mc;
...
SELECT vl.PAT_ID,
       CASE 
	   WHEN vl.Result_Output > 1000 THEN 3
	   WHEN vl.Result_Output > 199 THEN 1
           ELSE 0
       END AS HIV_VL,
...
/*Diabetes*/
SELECT dm.PAT_ID,
       CASE WHEN dm.HBA1C_LAST IS NULL THEN 3
           WHEN dm.HBA1C_LAST > 9.0 THEN 3
           ELSE 1
       END AS HBA1c,
       dm.HBA1C_LAST
INTO #dm
...
SELECT 
	copd.PAT_ID
	,MAX(CASE
		WHEN i.CODE = 'J96.11' THEN 3
		ELSE 1
	END) AS COPD
	,MAX(CASE
		WHEN i.CODE = 'J96.11' THEN 'Group D'
		ELSE 'Group A-C'
	END) AS 'HAS COPD' 
...
/*CAD*/
SELECT c.PAT_ID,
       MAX(CASE WHEN gcrl.BASE_GROUPER_ID = '2100000168' THEN 3 ELSE 1 END) AS HVD,
...
/*CKD or Hemodialysis*/
SELECT c.PAT_ID,
       MAX(CASE WHEN i.CODE IN ('N18.4', 'N18.5', 'N18.6')  THEN 3 --Stage 4
               WHEN gcrl.BASE_GROUPER_ID = '972013' THEN 3                    -- Dialysis
               ELSE 1
           END) AS CKD,
       MIN(CASE WHEN i.CODE IN ('N18.4', 'N18.5', 'N18.6')  THEN 'CKD Stage IV' --Stage 4
...
/*Obesity*/
SELECT c.PAT_ID,
       CASE WHEN w.BMI_LAST > 40.0 THEN 3
...
SELECT c.PAT_ID,
       MAX(CASE WHEN icd10.CODE = 'F03.90' THEN 3 --unspec. dementia  (placeholder until Leslie ges me an update)
...
SELECT c.PAT_ID,
       CASE WHEN meas.MEAS_VALUE = 'Yes, it has kept me from medical appointments or getting medications' THEN 3
...
/* Financial Resources*/
SELECT c.PAT_ID,
       CASE WHEN meas.MEAS_VALUE = 'Very Hard' THEN 3
...
SELECT c.PAT_ID,
       ...
       CASE WHEN sud.[Substance Use] IS NULL THEN 0
           WHEN sud.[Substance Use] = 4 THEN 3 --Since we're not adding tobacco and AODA together
...
SELECT fs.IDENTITY_ID MRN,
       ...
       CASE WHEN fs.[Psychiatric Diagnosis] + fs.[Substance Use] + fs.NUM_OF_MEDS + fs.HIV_VL + fs.HBA1c + fs.BP + fs.COPD + fs.CI + fs.HVD + fs.CKD + fs.BMI
                 + fs.HOUSING + fs.TRANSPORT_NEEDS + fs.FINANCIAL_STRAIN > 12 THEN '4. High-Risk'
           WHEN fs.[Psychiatric Diagnosis] + fs.[Substance Use] + fs.NUM_OF_MEDS + fs.HIV_VL + fs.HBA1c + fs.BP + fs.COPD + fs.CI + fs.HVD + fs.CKD + fs.BMI
                + fs.HOUSING + fs.TRANSPORT_NEEDS + fs.FINANCIAL_STRAIN > 8 THEN '3. Rising-Risk'
           WHEN fs.[Psychiatric Diagnosis] + fs.[Substance Use] + fs.NUM_OF_MEDS + fs.HIV_VL + fs.HBA1c + fs.BP + fs.COPD + fs.CI + fs.HVD + fs.CKD + fs.BMI
                + fs.HOUSING + fs.TRANSPORT_NEEDS + fs.FINANCIAL_STRAIN > 3 THEN '2. Low-Risk'
           ELSE '1. Healthy'
