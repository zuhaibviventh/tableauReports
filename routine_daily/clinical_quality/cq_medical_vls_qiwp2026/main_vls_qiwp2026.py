...
sql_file = f"{directory}/cq_medical_vls_qiwp2026/sql/vls_qiwp.sql"
...
def run(shared_drive):
    vls_logger.info(
        "Clinical Quality - Viral Load Suppression - QIWP 2026."
    )
    hyper_file = f"{shared_drive}/Vial Load Suppression QIWP 2026.hyper"
    ...
    try:
        ...
        if len(vls_df.index) == 0:
            ...
            vls_logger.info("Clinical Quality - Viral Load Suppression QIWP 2026 Daily ETL finished.")
        else:
...
def tableau_push(df, hyper_file):
    ...
    table_definition = TableDefinition(
        table_name = TableName("Viral Load Suppression QIWP 2026"),
        columns = [
    ...
    vls_logger.info(
        "Clinical Quality - Viral Load Suppression QIWP 2026 pushed to Tableau."
    )
