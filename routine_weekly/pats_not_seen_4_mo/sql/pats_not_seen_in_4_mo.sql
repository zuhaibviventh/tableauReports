...
WHERE a2.ROW_NUM_DESC = 1
      AND a2.PAT_ID IN ( SELECT DISTINCT pev.PAT_ID
                         ...
                         WHERE ser.SERV_AREA_ID = 64
                               AND ser.PROVIDER_TYPE_C IN ( '1', '6', '9', '113' ) -- Physicians and NPs
...
WHERE orv.RESULT_DATE > DATEADD(MONTH, -36, GETDATE()) --Need this to be long ago
      ...
      AND orv.COMPONENT_ID IN --(16705, 5543, 64338, 26518, 11688, 63500, 2532, 15737, 85928, 95256, 5037, 61505, 4661, 970107, 20525) Not needed any more since using COMMON NAME
...
WHERE a.LAST_OFFICE_VISIT < DATEADD(MONTH, -4, GETDATE());
...
SELECT b.MRN,
       ...
       b.ORD_VALUE 'LAST VL',
       CASE WHEN b.MOST_RECENT_Result_Output < 200 THEN 'SUPPRESSED'
           ELSE 'UNSUPPRESSED'
