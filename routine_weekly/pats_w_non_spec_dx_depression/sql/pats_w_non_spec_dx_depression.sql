...
SELECT a3.IDENTITY_ID,
       ...
       p.PAT_NAME,
       ser.PROV_NAME PCP,
       CASE WHEN icd10.CODE IN ( 'F32.9', 'F32.8' )
                THEN 0
       ELSE 1
       END AS MET_YN,
...
WITH flags AS (
    SELECT DISTINCT id.IDENTITY_ID,
                    a4.PAT_ID,
                    a4.PAT_NAME,
                    a4.PCP,
                    ...
                    a4.CITY,
                    MAX(
                    CASE WHEN ser2.PROVIDER_TYPE_C IN ( '136', '164', '129' )
                         THEN ser2.PROV_NAME END) AS 'PSYCHIATRY',
                    MAX(
                    CASE WHEN ser2.PROVIDER_TYPE_C NOT IN ( '136', '164',
                                                            '129' )
                             THEN ser2.PROV_NAME
...
SELECT flags.*,
       CASE WHEN flags.MH_TEAM_MEMBER IS NULL
                 AND flags.PSYCHIATRY IS NOT NULL
                THEN 'Psych'
       WHEN flags.MH_TEAM_MEMBER IS NOT NULL
            AND flags.PSYCHIATRY IS NULL
           THEN 'MH'
       WHEN flags.MH_TEAM_MEMBER IS NULL
            AND flags.PSYCHIATRY IS NULL
           THEN 'Medical'
       END AS indicator
FROM flags;
