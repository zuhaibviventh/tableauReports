 ...
 *****   GENERAL INFO   *****
 ...
 Object Name:	Press Ganey Data for CM FP and Housing
 Create Date:	6/10/2019
 Created By:	Mitch Scoggins/ARCW
 System:		PES001SVR
 Requested By:	Press Ganey

 Purpose:		Extract a list of activity to trigger Social Services Surveys

 Description:	NOTE: Save as csv using the file name '33118[MMDDYYY]_PE'. Save to 'N:\Press Ganey\Data Files Sent to Press Ganey'
 ...
 *****  Modification History *****
 ...
 6/5/2020			Mitch				Alteryx
  7/31/2020			Mitch				Updating to new AOrg using 'Vivent Health'
11/4/2020			Mitch				Adding Legal services
12/13/2021			Mitch				Updating to accomodate PE Standardization Changes
6/13/2022			Mitch				Fixing an issue where the ClientID/MRN sometimes does not population in the 'MED_REC' column
10/10/2023          Benzon              Updated to accomodate Austin
...
SELECT MAX(CASE WHEN lang.SCPLanguageProficiency = 'Spanish' THEN 1 ELSE 0 END) AS LANGUAGE,
       lang.SQLID
INTO #Language
...
SELECT CASE WHEN sp.ServiceType LIKE 'Food%' THEN 'FOOD'
       ...
       CASE WHEN vcp.SCPClientMI = '' THEN NULL
           ELSE vcp.SCPClientMI
       END AS PAT_MIDDLE_NAME,
       ...
       */
       ...
       END AS MED_REC,
       CASE WHEN sp.ServiceType LIKE 'Food%' THEN 'the Food Pantry staff'
           ELSE sp.ServiceProvider
       END AS PROV_NAME,
       ...
       CASE WHEN vcp.SCPPermissionEmail = 'Yes' THEN vcp.SCPEmailAddr
           ELSE NULL
...
SELECT CASE WHEN a.SERV_TYPE = 'CM'
       ...
       a.STATE,
       a.ZIP,
       CASE WHEN a.HOME_PHONE LIKE '___-___-____' THEN a.HOME_PHONE
           ELSE NULL
       END AS HOME_PHONE,
       a.UNIQUE_VISIT_ID,
       a.SVC_DATE,
