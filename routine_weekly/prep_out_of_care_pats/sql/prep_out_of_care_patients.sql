...
SELECT p.PAT_NAME 'Patient_Name',
       ...
       id.IDENTITY_ID 'MRN',
       CONVERT(VARCHAR, (p.BIRTH_DATE), 101) 'DOB',
       (DATEDIFF(m, P.BIRTH_DATE, GETDATE()) / 12) 'Current_Age',
       zcs.ABBR 'Gender',
       zpr.NAME 'Patient_Race',
       zeg.NAME 'Ethnicity',
       CONVERT(VARCHAR, (labs.[Last HIV Lab Date]), 101) [Last HIV Lab Date],
       ...
       smt.Last_PrEP_Retention_Selection_Visit_Date,
       ...
       CASE WHEN smt.Last_PrEP_Retention_Selection = 'Indicated for PrEP and Not Retained in care' THEN smt.Last_PrEP_Non_Retention_Reason_Visit_Date
           ELSE NULL
       END AS Last_PrEP_Non_Retention_Reason_Visit_Date,
       CONVERT(VARCHAR, (pcp.Next_Visit), 101) Next_Visit,
       ...
       CONVERT(VARCHAR, (GETDATE() - 1), 101) 'Report_Period_End',
...
SELECT Patient_Name,
       ...
       Last_PrEP_Retention_Selection_Visit_Date,
       Last_PrEP_Non_Retention_Reason,
       Last_PrEP_Non_Retention_Reason_Visit_Date,
       CURRENT_PCP_VAME,
       PAT_PREP_STATUS,
       Report_Period_End,
       CURRENT_ZIP_CODE,
       ...
       [MONTHS SINCE SEEN]
FROM #Final_Records
WHERE (Last_PrEP_Retention_Selection NOT IN ( 'Indicated for PrEP and Not Retained in care', 'Lost to Follow-up for PrEP care',
                                              'No longer Indicated for PrEP care' )
       AND Last_PrEP_Retention_Selection NOT LIKE 'Indicated for PrEP and Not Retained in care%'
       OR Last_PrEP_Retention_Selection IS NULL)
      AND CURRENT_PCP_VAME <> 'No Current PCP'
      AND Next_Visit IS NULL
      AND DATEDIFF(MONTH, Last_Visit, GETDATE()) >= 4
      AND DATEDIFF(MONTH, [Last HIV Lab Date], GETDATE()) >= 4
      AND MRN NOT IN ( SELECT ##excluded_mrns.IDENTITY_ID FROM ##excluded_mrns );
